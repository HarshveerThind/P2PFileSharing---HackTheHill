...
def main():
    ...
    # Print the chunks that Node 1 has
    print(f"Node 1 has chunks: {list(available_chunks.keys())}")
    ...
    if None not in downloaded_chunks:
        ...
        print(f"Node 1 is reconstructing the file with chunks: {list(range(len(downloaded_chunks)))}")
        rebuild_file(downloaded_chunks, 'reconstructed_file_node1.txt')
        print("File successfully reconstructed by Node 1.")
    else:
        print("Failed to download all chunks.")

if __name__ == '__main__':
    threading.Thread(target=start_server, args=(8000, chunk_file('file_to_share.txt'))).start()
    time.sleep(2)
    main()
