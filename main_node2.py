...
def main():
    ...
    available_chunks = {i: file_chunks[i] for i in range(3, 6) if i < len(file_chunks)}  # Ensure we don't exceed the chunk list

    # Print the chunks that Node 2 has
    print(f"Node 2 has chunks: {list(available_chunks.keys())}")
    ...
    if None not in downloaded_chunks:
        ...
        print(f"Node 2 is reconstructing the file with chunks: {list(range(len(downloaded_chunks)))}")
        rebuild_file(downloaded_chunks, 'reconstructed_file_node2.txt')
        print("File successfully reconstructed by Node 2.")
    else:
        print("Failed to download all chunks.")

if __name__ == '__main__':
    threading.Thread(target=start_server, args=(8001, chunk_file('file_to_share.txt'))).start()
    time.sleep(2)
    main()
